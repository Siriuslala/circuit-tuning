...
def get_relevant_params(model, top_edges, bias=True):
    ...
    for node in downstream_nodes:
        ...
        qkv_map = {"q": ["W_Q", "b_Q"], "k": ["_W_K", "_b_K"], "v": ["_W_V", "_b_V"]} if bias else {"q": ["W_Q"], "k": ["_W_K"], "v": ["_W_V"]}
        if "mlp" in node:
            layer_id = node.split(".")[1]
            head_id = node.split(".")[2]
            module_name = f"blocks.{layer_id}.mlp"
            param_names = ["W_in", "W_gate", "b_in", "b_gate"] if bias else ["W_in", "W_gate"]
            for param_name in param_names:
                param_key = f"{module_name}.{param_name}_{head_id}"
                relevant_params.append(param_key)
        elif "head" in node:
...
def circuit_tuning(
    ...
    if config.topn_scheduler[1] != 0:
        topn_scheduler = TopNScheduler(
            start_val=config.topn_scheduler[0], 
    ...
    train_steps_per_epoch = len(train_dataloader) // config.batch_size
    ...
    for epoch in range(1, config.num_epochs + 1):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc=f"Training Epoch {epoch}", colour="green")):
            ...
            if config.objective == "standard" or config.loss_weight_p == 1:
                # in normal fwd, eap_func does not add the reg term, so we need to add it here
                if config.task == "bias":
                ...
                if config.objective == "weighted" and config.task in ["sv"]:
                    ...
                    weighted_loss = weighted_loss_func(
                        logits=logits,
            ...
            if step % config.accumulation_steps == 0 or step == len(train_dataloader) - 1:
                ...
                # print("relevant_params:", relevant_params)
                # print("irrelevant_params:", irrelevant_params)
                # print("top_edges:", top_edges)
                # print("edge_num:", len(top_edges))
                # print("top_n:", config.top_n)
                # breakpoint()
                ...
                param_info_dict["real_tuning_params_num"].append(real_num)
                param_info_dict["real_tuning_params_mem"].append(real_mem)
                param_info_dict["nodes_num"].append(nodes_num)
                real_param_ratio = real_num / model_params_num
                param_info_for_log = {
                    "real_tuning_params_num": real_num,
                    "real_tuning_params_mem": real_mem,
            ...
            # save at the last step of the epoch
            # if step == train_steps_per_epoch - 1:
            #     save_model(model, config, fsdp_config, epoch=epoch, save_dir=save_dir, rank=rank, ckpt_name=f"model-epoch_{epoch}.pt")
...
def load_llama_in_HookedTransformer(train_config, fsdp_config):
    ...
    (2) Device    cfg.device
    """
    tokenizer = AutoTokenizer.from_pretrained(train_config.model_name, local_files_only=True)
    tokenizer.pad_token = tokenizer.eos_token
    tokenizer.padding_side = "left"
    
    if train_config.ckpt_path:
        cfg_path = os.path.join(train_config.model_name, "config.json")
        tokenizer = AutoTokenizer.from_pretrained(train_config.model_name)
        tokenizer.pad_token = tokenizer.eos_token
        model = load_model_from_ckpt(
            official_model_name=train_config.model_name,
        ...
        model = LlamaForCausalLM.from_pretrained(
            train_config.model_name,  # train_config.model_path
    ...
    if not train_config.enable_fsdp:
        ...
        model.cfg.device = train_config.device
...
def prepare_data(train_config, tokenizer):
    ...
    if train_config.task == "math":
        dataset_config.file = str(root_dir / "data/gsm8k_dataset.py")
    elif train_config.task == "bias":
        dataset_config.file = str(root_dir / "data/bias_dataset.py")
    elif train_config.task == "logic":
        dataset_config.file = str(root_dir / "data/logic_dataset.py")
    elif train_config.task == "reading":
        dataset_config.file = str(root_dir / "data/squad_dataset.py")
    else:
...
def train():
    ...
    print_rank_0("num_steps_per_epoch: ", len(train_dataloader))
