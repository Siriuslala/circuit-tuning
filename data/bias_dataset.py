...
def get_custom_dataset(dataset_config, tokenizer, split: str):
    ...
    if split == "train":
        return BiasDataset(data_path=root_dir / "data/bias/gender_bias/gender_bias_formatted_llama.jsonl",
                           tokenizer=tokenizer)
    elif split == "test":
        return None
    else:
...
if __name__ == "__main__":
    ...
    tokenizer = AutoTokenizer.from_pretrained(root_dir / "hf/models--meta-llama--Llama-3.2-1B-instruct")
    ...
    dataset = BiasDataset(data_path=root_dir / "data/bias/gender_bias/gender_bias_formatted_llama.jsonl",
                           tokenizer=tokenizer)
    lengths = []
    for data in dataset:
