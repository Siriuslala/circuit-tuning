...
class EAPGraph:
    ...
    def top_edges(
        ...
        if prune_method == "top_edges":
        ...
        elif prune_method == "top_nodes":
            # wrong implementation, deprecated
            ...
            top_scores, top_indices = torch.topk(self.eap_scores.flatten(), k=n, dim=0)
        elif prune_method == "top_nodes_new":
            if abs_scores:
                upstream_node_importance = self.eap_scores.abs().sum(dim=1, keepdim=True)
            else:
                upstream_node_importance = self.eap_scores.sum(dim=1, keepdim=True)

            if abs_scores:
                downstream_node_importance = self.eap_scores.abs().sum(dim=0, keepdim=True)
            else:
                downstream_node_importance = self.eap_scores.sum(dim=0, keepdim=True)

            #  New_Score(u, v) = Score(u, v) * Importance(u) * Importance(v)
            # Broadcasting：[N, M] * [N, 1] * [1, M] -> [N, M]
            if abs_scores:
                weighted_scores = self.eap_scores.abs() * upstream_node_importance * downstream_node_importance
            else:
                weighted_scores = self.eap_scores * upstream_node_importance * downstream_node_importance

            if abs_scores:
                top_scores, top_indices = torch.topk(weighted_scores.flatten().abs(), k=n, dim=0)
            else:
                top_scores, top_indices = torch.topk(weighted_scores.flatten(), k=n, dim=0)
            ...
            for i, (abs_score, index) in enumerate(zip(top_scores, top_indices)):
                ...
                top_edges.append((self.upstream_nodes[upstream_node_idx], self.downstream_nodes[downstream_node_idx], score.item()))
        elif prune_method == "top_nodes_new_exp":
            if abs_scores:
                upstream_node_importance = self.eap_scores.abs().sum(dim=1, keepdim=True)
            else:
                upstream_node_importance = self.eap_scores.sum(dim=1, keepdim=True)
            ...
            if abs_scores:
                # weighted_scores = self.eap_scores.abs() * upstream_node_importance * downstream_node_importance
                # change a*b*c to exp(loga + logb + logc)
                weighted_scores = torch.exp(torch.log(self.eap_scores.abs()) + torch.log(upstream_node_importance) + torch.log(downstream_node_importance))
            else:
                weighted_scores = torch.exp(torch.log(self.eap_scores) + torch.log(upstream_node_importance) + torch.log(downstream_node_importance))
            ...
            for i, (abs_score, index) in enumerate(zip(top_scores, top_indices)):
                ...
                top_edges.append((self.upstream_nodes[upstream_node_idx], self.downstream_nodes[downstream_node_idx], score.item()))
        elif prune_method == "top_nodes_new1":
            ...
            #     downstream_node_scores[down_stream_node] = self.eap_scores[:, down_stream_idx].abs().sum()
            # node_scores = {**upstream_nodes_scores, **downstream_node_scores}
            # top_nodes = sorted(node_scores.items(), key=lambda x: x[1], reverse=True)[:50]
            # # for each node, get the edge with the highest score that connects to it
            # top_edges = []
...
if __name__ == "__main__":
    ...
    def draw_graph_for_every_log_step():
        ...
        log_path = work_dir / "checkpoints-new/gpt2-small-sv-epochs_3-bsz_16-lr_1e-3-Opt_{SGD}-warm_up_100-top_n_1000-threshold_0-random_ratio_0-ablation_{mean}-loss_weighted(p_1)/log.jsonl"
        topn=1000
        select = 1000
        graph_dir = root_dir / f"figures/eap_graph/eap_graph_svd_topn_{topn}_select_{select}"
        ...
        edge_score_path = graph_dir / "edge_scores.jsonl"
    ...
    def analyze_hebbian_learning_sv_svd():
        ...
        sv_path = root_dir / "figures/eap_graph/sv_topn_50_sel_35_edge_info.jsonl"
        with jsonlines.open(sv_path, "r") as f:
            lines = [line for line in f]
        sv_edges = lines[0]["edges"]
        
        edge_info_path = root_dir / "figures/eap_graph/eap_graph_svd_topn_50_select_35/edge_scores.jsonl"
    ...
    def top_edges_vs_top_nodes():
        log_path = work_dir / "checkpoints-new/gpt2-small-sv-epochs_3-bsz_16-lr_1e-3-Opt_{SGD}-warm_up_100-top_n_5000-threshold_0-random_ratio_0-ablation_{mean}-loss_weighted(p_1)/log.jsonl"
        top_edges_svd = None
        with jsonlines.open(log_path, "r") as f:
            for line in f:
                if "edge_info" in line:
        ...
        top_edges_svd = sorted(top_edges_svd, key=lambda x: x[-1], reverse=True)
        topn_edges = top_edges_svd[:5000]
        svd_nodes = set()
        svd_edges = set()
        for edge in topn_edges:
            up, down, score = edge
            down = down.replace(".q", "").replace(".k", "").replace(".v", "")
            svd_nodes.add(up)
            svd_nodes.add(down)
            svd_edges.add((up, down))
        print("svd_nodes num:", len(svd_nodes))  # 1000 edges ~ 125; 100 edges ~ 39
        print("head.6.0 in svd_nodes:", "head.6.0" in svd_nodes)
        print("head.6.0 in top_edges_svd:", list(topn_edges.keys()).index("head.6.0"))
        
        from collections import defaultdict
        node_sum_scores = defaultdict(float)
        for edge in top_edges_svd:
            up, down, score = edge
            down = down.replace(".q", "").replace(".k", "").replace(".v", "")
            node_sum_scores[up] += score
            node_sum_scores[down] += score
        top_nodes_svd = sorted(node_sum_scores.items(), key=lambda x: x[1], reverse=True)
        top_nodes_svd = dict(top_nodes_svd)
        print("head.6.0 in top_nodes_svd:", list(top_nodes_svd.keys()).index("head.6.0"))
    
    def top_edges_vs_top_nodes_llama():
        log_path = work_dir / "checkpoints-math/1b-topn2000.jsonl"
        top_edges_svd = None
        with jsonlines.open(log_path, "r") as f:
            for line in f:
                if "top_edges" in line:
                    top_edges_svd = line["top_edges"]
                    break
        
        top_edges_svd = sorted(top_edges_svd, key=lambda x: abs(x[-1]), reverse=True)
        topn_edges = top_edges_svd[:]
        svd_nodes = set()
        svd_edges = set()
        for edge in topn_edges:
            up, down, score = edge
            ...
            svd_nodes.add(down)
            svd_edges.add((up, down))
        print("svd_nodes num:", len(svd_nodes))  # 2000 edges ~ 125
        
    # draw_graph_for_every_log_step()
    # analyze_hebbian_learning_sv_svd()
    # check_graph()
    # top_edges_vs_top_nodes()