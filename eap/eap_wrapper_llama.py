...
def EAP_ablation_multi_step(
    ...
    device = torch.cuda.current_device() if config.enable_fsdp else config.device
    ...
    # print("EAP scores:")
    # print(graph.eap_scores[0])
    # print(graph.eap_scores[14])
    # print(graph.eap_scores[32])
    # breakpoint()
...
if __name__ == "__main__":
    ...
    def compare_sv_svd():
        ...
        for batch in dataloader:
            ...
            # get svd scores
            import jsonlines
            log_path = root_dir / "checkpoints-graph/gpt2-small-sv-epochs_3-bsz_16-lr_1e-3-Opt_{SGD}-warm_up_100-top_n_50-threshold_0-random_ratio_0-ablation_{mean}-loss_weighted(p_1)/log.jsonl"
            steps = []
            top_edges_svd = None
            ...
            # log_path = root_dir / "checkpoints-new/gpt2-small-sv-epochs_3-bsz_16-lr_1e-3-Opt_{SGD}-warm_up_100-top_n_50-threshold_0-random_ratio_0-ablation_{mean}-loss_weighted(p_1)/log.jsonl"
            # data = []
            # with jsonlines.open(log_path, "r") as f:
            #     for line in f:
            #         data.append(line)
            ...
            _, _ = graph.show(edges=top_edges, key_nodes=key_nodes, key_edges=key_edges, fname=f"eap_graph_sv_topn_{top_n}_1.pdf", fdir=str(root_dir / "figures/graph_sv_svd"))
            _, _ = graph.show(edges=top_edges_svd, key_nodes=key_nodes, key_edges=key_edges, fname=f"svd_graph_svd_topn_{top_n}_1.pdf", fdir=str(root_dir / "figures/graph_sv_svd"))
            break
    ...
    def get_sv_graph():
        ...
        train_dataset = SVDataset(str(root_dir / "data/sv_dataset/train_24000.jsonl"), model.tokenizer)
        sv_collate_fn = SVCollateFn(model.tokenizer)
        dataloader = DataLoader(train_dataset, batch_size=64, shuffle=True, collate_fn=sv_collate_fn)
        ...
        _, edges = graph.show(edges=top_edges, fname=f"eap_graph_sv_topn_{top_n}_sel_35.pdf", fdir=str(root_dir / "eap/ims"))
