...
def EAP_IG_ablation(
    ...
    def capture_clean_hook(act, hook):
        # cache_clean[hook.name] = act.detach()
        if config.ablation_method == "zero":
            cache_corrupted[hook.name] = torch.zeros_like(act)
        else:
            cache_corrupted[hook.name] = act.mean(dim=0, keepdim=True).expand_as(act)

    upstream_hook_filter = lambda name: name.endswith(tuple(graph.upstream_hooks))
    model.add_hook(upstream_hook_filter, capture_clean_hook, "fwd")
    
    with torch.no_grad():
        model(input=clean_input_ids, attention_mask=clean_attention_mask)
    model.reset_hooks()

    # --- Step 2: Initialize A_diff ---
    global upstream_activations_difference
    ...
    forward_hook_fn = forward_hook_func_map[config.task]
    model.add_hook(upstream_hook_filter, partial(
        forward_hook_fn, 
        graph=graph, 
        batch=batch, 
        ie_over_seq=config.ie_over_seq, 
    ...
    # --- Step 3: IG loop (sample gradients) ---
    backward_hook_fn = backward_hook_func_map[config.task]
    downstream_hook_filter = lambda name: name.endswith(tuple(graph.downstream_hooks))
    ...
    for step in range(1, ig_steps + 1):
        ...
        # A_diff * Grad 
        model.add_hook(downstream_hook_filter, partial(
            backward_hook_fn, 
            graph=graph, 
            batch=batch, 
            config=config,
        ...
        value = metric(logits=logits, clean_logits=logits, corrupted_logits=None, batch=batch)
        
        value.backward(retain_graph=(True))

        # print("Step:", step, "Value:", value.item())
        # print("EAP scores:", graph.eap_scores)
        # breakpoint()
...
if __name__ == "__main__":
    ...
    def compare_sv_svd_old_old(model):
        ...
        train_dataset = SVDataset(root_dir / "data/sv_dataset/train_24000.jsonl", model.tokenizer)
        ...
        for batch in dataloader:
            ...
            # get svd scores
            import jsonlines
            log_path = work_dir / "checkpoints-sv/gpt2-small-sv-epochs_3-bsz_16-lr_1e-3-Opt_{SGD}-warm_up_100-top_n_50-threshold_0-random_ratio_0-ablation_{mean}-loss_weighted(p_1)/log.jsonl"
            steps = []
            top_edges_svd = None
            ...
            # log_path = work_dir / "checkpoints-new/gpt2-small-sv-epochs_3-bsz_16-lr_1e-3-Opt_{SGD}-warm_up_100-top_n_50-threshold_0-random_ratio_0-ablation_{mean}-loss_weighted(p_1)/log.jsonl"
            # data = []
            # with jsonlines.open(log_path, "r") as f:
            #     for line in f:
            #         data.append(line)
            ...
            _, _ = graph.show(edges=top_edges, key_nodes=key_nodes, key_edges=key_edges, fname=f"eap_graph_sv_topn_{top_n}_1.pdf", fdir=root_dir / "figures/graph_sv_svd_new")
            _, _ = graph.show(edges=top_edges_svd, key_nodes=key_nodes, key_edges=key_edges, fname=f"svd_graph_svd_topn_{top_n}_1.pdf", fdir=root_dir / "figures/graph_sv_svd_new")
            break
    ...
    def compare_sv_svd(top_n=70):
        ...
        train_dataset = SVDataset(work_dir / "data/sv_dataset/test_3000.jsonl", model.tokenizer)
        sv_collate_fn = SVCollateFn(model.tokenizer)
        dataloader = DataLoader(train_dataset, batch_size=64, shuffle=True, collate_fn=sv_collate_fn)
        ...
        ckpt_path = work_dir / "checkpoints-new/gpt2-small-sv-epochs_3-bsz_16-lr_1e-3-Opt_SGD-warm_up_100-top_n_1000-threshold_0-metric-logit_diff-random_ratio_0-randn_0-ablation_mean-loss_weighted_p_1-prune_method_top_nodes/model-steps_1499_epoch_3.pt"
        ...
        _, _ = graph.show(edges=top_edges, key_nodes=key_nodes, key_edges=key_edges, fname=f"eap_graph_sv_topn_{top_n}_1.pdf", fdir=root_dir / "figures/graph_lyy")
        _, _ = graph.show(edges=top_edges_svd, key_nodes=key_nodes, key_edges=key_edges, fname=f"eap_graph_svd_topn_{top_n}_1.pdf", fdir=root_dir / "figures/graph_lyy")
    ...
    def get_sv_graph(top_n):
        ...
        _, edges = graph.show(edges=top_edges, fname=f"eap_graph_sv_topn_{top_n}_new.pdf", fdir=root_dir / "figures/graph_sv_svd_new")

        # edge_info_path = os.path.join(root_dir / "eap/ims", "sv_topn_50_sel_35_edge_info.jsonl")
        # import jsonlines
    ...
    def get_svd_graph(top_n):
        ...
        _, edges = graph.show(edges=top_edges, fname=f"eap_graph_svd_topn_{top_n}_new.pdf", fdir=root_dir / "figures/graph_sv_svd_new")
