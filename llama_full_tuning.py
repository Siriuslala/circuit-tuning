...
args["ckpt_name"] = f"{simple_model_name}-{args['task']}-epochs_{args['num_epochs']}-bsz_{bsz}-lr_{args['lr']}-Opt_SGD-full-precision_{args['precision']}-1"
...
train_config.enable_fsdp = args["enable_fsdp"]
...
model = LlamaForCausalLM.from_pretrained(
    ...
    # token=MODEL_TOKEN_MAP.get(train_config.model_name, None),
)
...
if train_config.enable_fsdp:
    if fsdp_config.pure_bf16 and not train_config.quantization:
...
if not args["enable_fsdp"] or (args["enable_fsdp"] and rank == 0):
