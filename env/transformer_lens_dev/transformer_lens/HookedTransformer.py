...
class HookedTransformer(HookedRootModule):
    ...
    def __init__(
        ...
        if not split_params:
            ...
            self.blocks = nn.ModuleList(
                [TransformerBlockSplit(self.cfg, block_index) for block_index in range(self.cfg.n_layers)]
            )
            self.cfg.n_mlp_head = self.blocks[0].n_mlp_head
    ...
    def input_to_embed(
        ...
        if len(tokens.shape) == 1:
            ...
            tokens = tokens[None]
        # print("In input_to_embed (before if): the device of tokens:", tokens.device)
        if tokens.device.type != self.cfg.device and self.move_to_device:
            tokens = tokens.to(devices.get_device_for_block_index(0, self.cfg))
        # print("In input_to_embed (after if): the device of tokens:", tokens.device)
    ...
    def forward(
        ...
        with utils.LocallyOverridenDefaults(
            ...
            for i, block in blocks_and_idxs[start_at_layer:stop_at_layer]:  # type: ignore
                ...
                if shortformer_pos_embed is not None and self.move_to_device:
    ...
    def from_pretrained(
        ...
        **from_pretrained_kwargs,
        ...
        # Check whether to split_params
        split_params = from_pretrained_kwargs.get("split_params", False)
