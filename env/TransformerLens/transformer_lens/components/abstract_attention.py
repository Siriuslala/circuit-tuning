...
class AbstractAttentionSplit(ABC, nn.Module):
    ...
    @property
    def W_Q(self):
        return torch.stack([getattr(self, f'W_Q_{i}') for i in range(self.cfg.n_heads)], dim=0)
    
    @property
    def W_O(self):
        return torch.stack([getattr(self, f'W_O_{i}') for i in range(self.cfg.n_heads)], dim=0)
    ...
    def forward(
        ...
        if not self.cfg.use_attn_result:
            if self.cfg.load_in_4bit:
                ...
                w = einops.rearrange(
                    torch.stack([getattr(self, f'W_O_{i}') for i in range(self.cfg.n_heads)], dim=0), "head_index d_head d_model -> d_model (head_index d_head)"
                )
                out = F.linear(
                    z.reshape(z.shape[0], z.shape[1], self.cfg.d_head * self.cfg.n_heads),
                    w,
            ...
            if self.cfg.load_in_4bit:
                ...
                w = einops.rearrange(
                    torch.stack([getattr(self, f'W_O_{i}') for i in range(self.cfg.n_heads)], dim=0),
