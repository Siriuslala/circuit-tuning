...
# Transformer Block, with attention heads and mlps spilted
...
class TransformerBlockSplit(nn.Module):
    ...
    def __init__(self, cfg: Union[Dict, HookedTransformerConfig], block_index):
        ...
        attention = Attention if self.cfg.n_key_value_heads is None else GroupedQueryAttentionSplit
        if not self.cfg.use_local_attn:
            self.attn = attention(self.cfg, "global", block_index)
        else:
            if self.cfg.attn_types is None:
        ...
        if not self.cfg.attn_only:
            self.mlp = MLPFactory.create_mlp(self.cfg, split_params=True)
    ...
    def forward(
        ...
        if not self.cfg.attn_only and not self.cfg.parallel_attn_mlp:
            ...
            normalized_resid_mid = self.ln2(mlp_in)
            mlp_out = self.apply_mlp(normalized_resid_mid)  # [batch, pos, n_mlp_head, d_mlp_head]
            # mlp_out = mlp_out.reshape(mlp_out.shape[0], mlp_out.shape[1], self.d_mlp)
            resid_post = self.hook_resid_post(resid_mid + mlp_out)  # [batch, pos, d_model]
        elif self.cfg.parallel_attn_mlp:
            # Dumb thing done by GPT-J, both MLP and Attn read from resid_pre and write to resid_post, no resid_mid used.
            # In GPT-J, LN1 and LN2 are tied, in GPT-NeoX they aren't.
...
c
        Returns:
            Float[torch.Tensor, "batch pos d_model"]: Our resulting tensor
        """
        mlp_out = self.hook_mlp_out(self.mlp(normalized_resid))  # [batch, pos, n_mlp_head, d_model]
        mlp_out = (
            einops.reduce(mlp_out, "batch pos n_mlp_head d_model -> batch pos d_model", "sum")
        )
        if self.cfg.use_normalization_before_and_after:
            mlp_out = self.ln2_post(mlp_out)
            
        # mlp_out = self.hook_mlp_out(mlp_out.reshape(mlp_out.shape[0], mlp_out.shape[1], self.n_mlp_head, self.d_mlp_head))  # batch pos n_mlp_head d_mlp_head
